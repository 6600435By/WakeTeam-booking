...
function welcome(name: string): TourStepDef {
  return {
    ...
    description:
      "Краткий тур покажет, как начать рабочий день. Можно пропустить — подробности всегда в «Спр.» (справка).",
  };
}
...
function helpStep(): TourStepDef {
  return {
    ...
    title: "Справка",
    description:
      "Пункт «Спр.» в меню — инструкции, сценарии и FAQ. Внизу страницы — «Повторить тур».",
    side: "bottom",
  };
}
